...
<div id="containerSR" style="background-color:green; margin-left:auto; margin-right:auto; width:1000px;">
...
<form name = "bookForm" method = "GET" action="searchServletProfile">
	...
	<div id="clear"></div>
    <h1 style="margin-left:25px;"><i>Search Results: ${param.searchText}</i></h1>
...
<script>
		...
		console.log("before callservlet function");
		console.log(rawdata);
		function callServlet(index){
			
			console.log("index is in callservlet:"+index);
			data = <%= session.getAttribute("data") %>;
			console.log("before setting bookid to string");
			console.log("BEFORE JQUERY CALL");
			
			$.ajax({
                url: 'toDetailsProfServlet',
                data: {
                    indexvar: index,
                    holder: rawdata.items[index].id
                },
                type: 'GET',
                success: function(data){
                  location.href = "detailsProfile.jsp?id="+index
                } 

            });
			console.log("AFTER JQUERY CALL");
		}
		...
		for(i=0; i<data.items.length; i++) {
			...
			$("#results").append('<table><tr><td rowspan="4">&nbsp'+'<img src="'+data.items[i].volumeInfo.imageLinks.smallThumbnail+'" onclick="callServlet('+i+')"/>'+'</td><td>&nbsp</td></tr>'
