...
int main(int argc, char* argv[])
{
    ...
    while(!done) {
	    ...
	    /* Add support for other commands here */
                ...
                if(ss>>username && ss>>hitnum ){
                    username=convToLower(username);
                    //if user is in the set and the hit num is between the range then add product to cart
                    if(ds.isUserinSet(username) && (hitnum>0 && hitnum<=hits.size())){
