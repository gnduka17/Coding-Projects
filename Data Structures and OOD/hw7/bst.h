...
protected:
    ...
    //helper function to clear the bst  
    void clearHelp(Node<Key, Value>* root){
        if(root!=NULL){
    ...
    //helper function to return the node with the smallest key  
    Node<Key, Value>* smallNodeHelp(Node<Key, Value>* root) const{
        if(root->getLeft()!=NULL){
            return smallNodeHelp(root->getLeft());
        }
        return root;
    }
    //returns height of given root  
...
BinarySearchTree<Key, Value>::iterator::operator++()
{
    ...
    {
        Node<Key, Value>* parent_ = current_->getParent();
        while(parent_ != NULL && current_ == parent_->getRight())
        {
            current_ = parent_;
...
{
    root_ = this->insertHelp(keyValuePair, root_);
}
