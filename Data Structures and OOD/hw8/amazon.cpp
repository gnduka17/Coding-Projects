...
unsigned long long hashfunc(string password);
...
int main(int argc, char* argv[])
{
    ...
    while(!done) {
            ...
            if(cmd=="LOGIN"){
                if(ss>>username){
                    ...
                    if(!ds.isUserinSet(username)){
                        ...
                        if(ds.correctHash(username)==hashfunc(password)){
                            username_on = true;
	    ...
	    /* Add support for other commands here */
            ...
            //     if no products exist to be rec
...
unsigned long long hashfunc(string password){
    ...
    unsigned long long num = 128;
    unsigned long long encryp;
    ...
    if(password.size()>8){
        ...
        for(unsigned int i =0;i<password.size();i++){
            output=output*num;
            unsigned long long lett=password[i];
            output=output+lett;
        }
    }
    for(int j=3; j>=0;j--){
        array[j]=output%65521;
        output=output/65521;
    }
    encryp=(45912 * array[0] + 35511 * array[1] + 65169 * array[2] + 4625 * array[3]) % 65521;
    return encryp;
}
